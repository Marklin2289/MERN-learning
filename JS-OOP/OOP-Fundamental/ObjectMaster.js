...
const pkmnIds = pokémon.map((p) => p.id).filter((num) => num % 3 === 0);
console.log(pkmnIds);

const fireType = pokémon.filter((p) => p.types.includes("fire"));
console.log(fireType);

const mulTypes = pokémon.filter((p) => p.types.length > 1);
...
const fireFlyingType = pokémon.filter((p) => p.types[1] === "flying");
console.log(fireFlyingType);

const normalTypeCount = pokémon.filter((p) => p.types.includes("normal"));
console.log("normal type count: ", normalTypeCount.length);
